...
const pizza = {
    ...
    popularity: 10,
    rating: 9.0,
    tags: ['gluten-free', 'cheese', 'no-onions']
}
console.log(pizza);
...
//CODE HERE
function filterByTag(foodsArr, filterTags) {
 return foodsArr.filter(food => food.tags.includes(filterTags));
}
console.log(filterByTag(foodArr, 'mashed'));
...
function filterByProperty(property, number) {
    const filteredArray = foodArr.filter(food => food[property] > number);
